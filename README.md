...
- **`index.html`** — the printable **one-page field guide** (also the site
  landing page): aerial + overlay, three QR codes (Google My Maps view · hosted
  KML · install the app), and the import + place-the-fence steps. Generated by
  `build-field-sheet.py`; `field-sheet.pdf` is the print/download version.
- **`map.html`** — a self-contained [Leaflet](https://leafletjs.com/) page on a
  Google satellite basemap. It renders the full overlay (operational arc in
  **red**, the bigger reference arcs in orange/cyan, diamond, foul lines,
  backstop, soccer penalty area, the 5 pins) with a **⌖ locate** button showing
  your live GPS position — open it on your phone at the field to walk the line.
  Basemap toggle (Google Satellite / Hybrid / Esri) top-right.
